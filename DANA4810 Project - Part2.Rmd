...
```
e. Paste the copied URL into the "Repository URL" text box back in RStudio. Then, click the "Create Project" button.<br>
    Note: The "Project directory name" field will automatically populate based on the repository name).<br>
```{r, echo=FALSE, out.width="70%", fig.align="left"}
...
To make sure that each team member's name shows up in Github after pushing, RStudio needs to be linked to appropriate Github account.<br>
...
#### 3.3 Pull the latest files from GitHub
```{r, echo=FALSE, out.width="80%", fig.align="left"}
knitr::include_graphics("33_Pull.png")
```

#### 3.4 Modify and Save files locally
```{r, echo=FALSE, out.width="80%", fig.align="left"}
knitr::include_graphics("34_Modify&Save.png")
```

#### 3.5 Commit your changes with a clear comment
```{r, echo=FALSE, out.width="80%", fig.align="left"}
knitr::include_graphics("35_Commit.png")
```
  Note:<br>
  - When making your first commit, RStudio may prompt you to enter your username and email address. If this occurs, please follow the steps in "2.4 Setup Up Account Information & Credentials" to complete the configuration.

#### 3.6 Push your commits to Github
```{r, echo=FALSE, out.width="80%", fig.align="left"}
...
a. Make and save your changes to the `.Rmd` file as normal.<br>
b. In RStudio, open the Git tab (top-right panel).<br>
c. Check the box next to each file you changed — this stages the file.<br>
d. Click Commit.<br>
e. In the box that appears, type a short message describing what you did.<br>
...
4. To see who wrote each specific line of a file, open the file and click **Blame** (top-right). GitHub will annotate every line with the author and the date it was last changed.<br>
```{r, echo=FALSE, out.width="80%", fig.align="left"}
...
Every commit message is visible to the whole team in the history. Writing clear messages
means your teammates always know what you were working on without having to ask.
